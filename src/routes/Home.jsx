...
import {
  ...
  CarouselPrevious,
  CarouselNext,
} from "../components/ui/carousel.jsx"; // Make sure it's .jsx if you're using JS
      ...
      <section className="py-12 md:py-16 lg:py-20 px-4 sm:px-6 lg:px-8">
        ...
        {newMovies.length > 0 ? (
          <Carousel
            ...
            opts={{ loop: newMovies.length > 3, align: "start" }}
            ...
            <CarouselContent className="-ml-4">
              {newMovies.map((movie) => (
                <CarouselItem key={movie.id} className="pl-4 basis-full sm:basis-1/2 md:basis-1/3 lg:basis-1/4 xl:basis-1/5 group">
                  <Link to={`/post/${movie.id}`} className="block h-full">
                    <div className="overflow-hidden rounded-lg shadow-xl transform group-hover:scale-105 transition-transform duration-300 bg-gray-800 h-full flex flex-col">
                      ...
                      <div className="p-3 md:p-4 flex flex-col flex-grow">
                        ...
                        <p className="text-xs text-gray-400 mb-2">{movie.release_date || 'N/A'}</p>
